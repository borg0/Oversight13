...
/obj/structure/girder/attackby(obj/item/W as obj, mob/user as mob)
	...
	else if(istype(W, /obj/item/weapon/pickaxe/policehammer))
		if(do_after(user,40))
			if(!src) return
			playsound(src, 'sound/items/wallhit.ogg', 100, 1)
			to_chat(user, "<span class='notice'>You break through the girder!</span>")
			dismantle()
